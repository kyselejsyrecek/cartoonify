...
class Workflow(object):
    ...
    def print_previous_original(self):
        """Print previous original. 
           When called multiple times, prints originals in backward order.
        """
        if self.self._original_image_number <= 0:
            self._logger.info(f'Refusing to print previous original as no prevous photo is available. Original image number: {self.self._original_image_number}.')
        else:
    ...
    def increment(self):
        ...
        self._original_image_number = (self._original_image_number + 1) % self._config.max_image_number
