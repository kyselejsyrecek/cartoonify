    ...
    def idle(self):
        ...
        try:
            if hasattr(self, '_exit_event') and self._exit_event and self._exit_event.is_set():
                if self._log:
                    self._log.info('Exit event detected in SayGui - closing application.')
                self.close()
        except Exception as e:
            if hasattr(self, '_log') and self._log:
                self._log.warning(f'Could not check exit_event in SayGui: {e}')
    
    def main(self, event_service, logger, exit_event, halt_event, i18n, cam_only):
        """Main entry point for SayGui.
        ...
        Args:
            ...
            cam_only: Boolean flag for camera-only mode (not used in Say UI).
        ...
        self._log.debug(f'SayGui.main() called, path={getattr(self, "path", "unknown")}')
