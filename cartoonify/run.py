...
@click.option('--host', default=None, help='Hostname or IP address to bind web server to. Defaults to system hostname. Use 0.0.0.0 to listen on all interfaces. If hostname supports subdomains, subordinate applications use subdomains (e.g., say.hostname).')
@click.option('--main-port', type=int, default=80, help='Port for main web application (MainWebGui). Defaults to 80.')
@click.option('--first-subordinate-port', type=int, default=2000, help='Starting port number for subordinate web applications (like SayWebGui) when subdomains are not available (default: 2000).')
@click.option('--cert-file', type=str, default=None, help='TLS certificate file for HTTPS server.')
@click.option('--key-file', type=str, default=None, help='TLS private key file for HTTPS server.')
...
def run(**kwargs):
    ...
    if config.debug:
        original_stdout, original_stderr, stderr_redirector = setup_debug_logging(log_level=log_level, use_colors=not config.no_log_colors, high_precision_time=config.debug_timing)
    else:
        original_stdout, original_stderr, stderr_redirector = setup_file_logging(logs_dir, log_level=log_level, redirect_stderr=True, use_colors=not config.no_log_colors, high_precision_time=config.debug_timing)
